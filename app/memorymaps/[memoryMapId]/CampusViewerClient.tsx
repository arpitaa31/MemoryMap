...
try { const nextCode = await reserveInviteCode(db); const batch = writeBatch(db); batch.update(doc(db, "memoryMaps", memoryMapId), { inviteCode: nextCode, updatedAt: serverTimestamp() }); batch.update(doc(db, "inviteCodes", map.inviteCode), { active: false, updatedAt: serverTimestamp() }); batch.set(doc(db, "inviteCodes", nextCode), { memoryMapId, active: true, createdBy: user.uid, ownerId: user.uid, mapName: map.name, ownerName: user.displayName ?? null, coverImageUrl: map.coverImageUrl ?? null, coverImagePosition: map.coverImagePosition, createdAt: serverTimestamp() }); await batch.commit(); setMap((previous) => previous ? { ...previous, inviteCode: nextCode } : previous); setInviteMessage("Invite link regenerated"); } catch { setInviteMessage("We could not regenerate the invite link."); }
  ...
  return <main className="mm-viewer">
    ...
    <section className="mm-viewer__campus-header" aria-label={`${map.name} campus header`}><div className="mm-viewer__shell mm-viewer__campus-header-inner"><div className="mm-viewer__campus-header-bg">{map.coverImageUrl && !coverImageFailed ? <img src={map.coverImageUrl} alt={`${map.name} campus cover`} width="1600" height="900" style={{ objectPosition: map.coverImagePosition }} onError={() => setCoverImageFailed(true)} /> : null}</div><div className="mm-viewer__campus-header-copy"><p className="mm-viewer__label">Private campus</p><h1>{map.name}</h1><p className="mm-viewer__member-status">{map.ownerId === user?.uid ? "Owner" : "Member"}</p></div>{map.ownerId === user?.uid && !user.isAnonymous && <button type="button" className="mm-viewer__cover-action" onClick={() => setCoverOpen(true)}>{map.coverImageUrl ? "Change cover" : "Add cover"}</button>}</div></section>

    <section className="mm-viewer__floor-navigation" aria-label="Floor selection"><div className="mm-viewer__shell mm-viewer__floor-navigation-inner"><span className="mm-viewer__label">Floors</span><div className="mm-viewer__floor-tabs" role="tablist" aria-label="Campus floors">{floors.map((item) => <button type="button" role="tab" key={item.floor.id} aria-selected={item.floor.id === current.floor.id} aria-current={item.floor.id === current.floor.id ? "page" : undefined} onClick={() => { setFloorId(item.floor.id); setRoomId(null); }}>{item.floor.name}</button>)}</div></div></section>
