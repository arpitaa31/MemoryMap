...
function toMemoryMapSummary(id: string, data: Record<string, unknown>, accessRole: "owner" | "member", userId: string): MemoryMapSummary {
  return {
    ...
    coverImageStorageId: typeof data.coverImageStorageId === "string" ? data.coverImageStorageId : null,
...
function CampusCard({ memoryMap, index, user, openMenuId, setOpenMenuId: setMenuState, onDelete, onCover }: { memoryMap: MemoryMapSummary; index: number; user: User; openMenuId: string | null; setOpenMenuId: (id: string | null) => void; onDelete: (memoryMap: MemoryMapSummary) => void; onCover: (memoryMap: MemoryMapSummary) => void }) {
  const isGuest = user.isAnonymous === true;
  ...
  const setOpenMenuId = (requested: string | null) => setMenuState(requested === null && openMenuId === null ? memoryMap.id : requested === memoryMap.id || requested === null ? requested : memoryMap.id);
  return <article className={`mm-campus-card mm-campus-card--${isSetup ? "setup" : "active"}${memoryMap.isShared ? " is-shared" : ""}`}><div className="mm-campus-card__top"><div className="mm-campus-card__identity"><span className={`mm-campus-card__status mm-campus-card__status--${memoryMap.isShared ? "shared" : isGuest ? "guest" : "owned"}`}><span />{memoryMap.isShared ? "Shared with you" : isGuest ? "Guest campus" : "Owned by you"}</span><h3><Link href={campusHref}>{memoryMap.name}</Link></h3>{memoryMap.schoolName && <p>{memoryMap.schoolName}</p>}{memoryMap.isShared && <p className="mm-campus-card__owner">From {memoryMap.ownerName || "a MemoryMap member"}</p>}</div>{isOwner && <div className="mm-card-menu"><button type="button" className="mm-card-menu__trigger" aria-label={`Actions for ${memoryMap.name}`} aria-haspopup="menu" aria-expanded={openMenuId === memoryMap.id} onClick={() => setOpenMenuId(openMenuId === memoryMap.id ? null : openMenuId)}><Icon name="menu" size={19} /></button>{openMenuId === memoryMap.id && <div className="mm-card-menu__panel" role="menu"><Link href={campusHref} role="menuitem" onClick={() => setOpenMenuId(null)}>{isSetup ? "Continue setup" : "Open campus"}</Link>{!isGuest && <Link href={`/memorymaps/${memoryMap.id}`} role="menuitem" onClick={() => setOpenMenuId(null)}>Invite people</Link>}<button type="button" role="menuitem" onClick={() => { setOpenMenuId(null); onCover(memoryMap); }}>{memoryMap.coverImageUrl ? "Change cover photo" : "Add cover photo"}</button><Link href={isSetup ? campusHref : `/memorymaps/${memoryMap.id}/setup`} role="menuitem" onClick={() => setOpenMenuId(null)}>Edit setup</Link><button type="button" role="menuitem" onClick={() => onDelete(memoryMap)}>Delete campus</button></div>}</div>}</div><Link href={campusHref} className="mm-campus-card__preview-link" aria-label={`${actionLabel}: ${memoryMap.name}`}><CampusCardVisual memoryMap={memoryMap} index={index} /></Link><div className="mm-campus-card__metrics"><span><Icon name="layers" size={15} /><b>{memoryMap.floorCount ?? "—"}</b> {memoryMap.floorCount === 1 ? "floor" : "floors"}</span><span><Icon name="campus" size={15} /><b>{memoryMap.roomCount}</b> {memoryMap.roomCount === 1 ? "room" : "rooms"}</span><span><Icon name="people" size={15} /><b>{memoryMap.memberCount}</b> {memoryMap.memberCount === 1 ? "member" : "members"}</span></div><div className="mm-campus-card__footer"><span><Icon name="clock" size={14} />{formatDate(memoryMap.updatedAt)}</span><Link href={campusHref}>{actionLabel}<Icon name="arrow" size={14} /></Link></div>{isSetup && <div className="mm-campus-card__setup-note"><Icon name="clock" size={14} />Setup incomplete</div>}</article>;
}
...
export default function DashboardClient() {
  ...
  const [coverCampus, setCoverCampus] = useState<MemoryMapSummary | null>(null);
