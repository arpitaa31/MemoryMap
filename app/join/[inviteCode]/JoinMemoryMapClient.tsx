...
export default function JoinMemoryMapClient({ inviteCode }: { inviteCode: string }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try { assertFirebaseConfig(); if (!db) throw new Error("Firestore unavailable"); const snapshot = await getDoc(doc(db, "inviteCodes", inviteCode)); const data = snapshot.data(); if (!snapshot.exists() || !data || data.active !== true || typeof data.memoryMapId !== "string") { setState("invalid"); return; } setInvite({ memoryMapId: data.memoryMapId, ownerId: typeof data.ownerId === "string" ? data.ownerId : "", mapName: typeof data.mapName === "string" ? data.mapName : "Private MemoryMap", ownerName: typeof data.ownerName === "string" ? data.ownerName : null, active: true }); setState("ready"); } catch { setState("error"); }
    };
    void load();
  ...
  const join = async () => {
    ...
    setState("joining");
    try { await setDoc(doc(db, "memoryMaps", invite.memoryMapId, "members", user.uid), { userId: user.uid, displayName: user.displayName ?? null, email: user.email ?? null, photoURL: user.photoURL ?? null, role: "member", status: "active", joinedAt: serverTimestamp(), inviteCode }, { merge: true }); router.push(`/memorymaps/${invite.memoryMapId}`); } catch { setMessage("We could not join this MemoryMap. Please try again."); setState("ready"); }
  };
  ...
  if (state === "invalid" || state === "error" || !invite) return <main className="mm-state-page"><h1>Invite link unavailable</h1><p>{state === "error" ? "We could not check this invite. Try again later." : "This invite link is no longer active or does not exist."}</p><Link href="/dashboard" className="mm-button mm-button--coral">Back to dashboard</Link></main>;
