...
export default function JoinMemoryMapClient({ inviteCode }: { inviteCode: string }) {
  ...
  if (state === "invalid" || state === "error" || !invite) return <main className="mm-state-page"><h1>Invite link unavailable</h1><p>{state === "error" ? "We could not check this invite. Try again later." : "This invite link is no longer valid."}</p><Link href="/dashboard" className="mm-button mm-button--coral">Back to dashboard</Link></main>;
